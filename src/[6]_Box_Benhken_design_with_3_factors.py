...
factors = {
    "min_samples_leaf": {-1: 25, 0: 50, 1: 75},
    "min_weight_fraction_leaf": {-1: 0.05, 0: 0.1, 1: 0.15},
...
def min_samples_leaf_f(x):
    return interpolate_encoded_values(x, 25, 50)


def min_weight_fraction_leaf_f(x):
    return interpolate_encoded_values(x, 0.05, 0.1)
...
RESLTS, RESULTS_INTERP = iterative_gradient_descent(
    best_combination, 0.01, model, model.coef_, 5, 0.1
)

print(RESULTS_INTERP["MSE"])
# %% Ridge analysis
...
for i in range(X1.shape[0]):
    for j in range(X1.shape[1]):
        Z[i, j] = model.predict(
            poly.transform(np.array([[X1[i, j], X2[i, j], 0]]))
        )[0]
...
# plt.title(
#     "Ridge plot centered at origin", fontsize=du.FONTSIZE_TITLE
# )
...
best_params = {
    "min_samples_leaf": 6,
    "min_weight_fraction_leaf": 0.061,
    "max_features": 6,
}
...
RESULTS_INTERP.to_excel(results_path + "/" + "[6] BB_results_R1.xlsx")
