...
file_path = "C:/Projects/MZIR/mzir_doe/datasets/Concrete_Data.xls"
...
rfr_model = make_pipeline(StandardScaler(), RandomForestRegressor(random_state=11))
for i, (train_idx, val_idx) in enumerate(kf5.split(X_train)):
    ...
    fitted_model = rfr_model.fit(Xtrain, ytrain.ravel())
...
rfr_model = make_pipeline(StandardScaler(), RandomForestRegressor(random_state=12))
...
print(f"Model MSE on entire dataset: {mean_squared_error(y_test, model_prediction)}")
